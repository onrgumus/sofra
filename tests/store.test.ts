...
describe('DemoStore', () => {
  ...
  it('seeds a company split across the configured offices', async () => {
    expect(await store.listEmployees()).toHaveLength(120);
    expect((await store.listEmployees(OFFICE)).length).toBeGreaterThan(0);
    expect((await store.listEmployees(OFFICE)).every((e) => e.officeId === OFFICE)).toBe(true);
  });
  ...
  it('seeds past lunches so the first real run already avoids repeats', async () => {
    expect((await store.listPastMatches()).length).toBeGreaterThan(0);
  });
  ...
  it('never seeds a lunch request for someone who is not in the office', async () => {
    ...
    for (const date of upcomingWeekdays(5, todayInZone('Europe/Istanbul'))) {
      const attending = new Set(await store.getAttendance(date, OFFICE));
      for (const optIn of await store.listOptIns(date, OFFICE)) {
        expect(attending.has(optIn.employeeId)).toBe(true);
      }
    }
  });

  it('gives everyone a distinct name', async () => {
  ...
  async function planLunches() {
    ...
    for (const optIn of optIns) await store.setOptIn(optIn);
  ...
  it('replaces the previous plan when matching is re-run', async () => {
    await planLunches();
    const firstIds = (await store.listGroups(date, OFFICE)).map((g) => g.id);
    await planLunches();
    const groups = await store.listGroups(date, OFFICE);

    // Same day matched twice must not accumulate two sets of tables.
    expect(groups.length).toBe(firstIds.length);
    expect((await store.listGroups(date, OFFICE)).every((g) => g.date === date)).toBe(true);
  });
  ...
  it('cancels a table once too many people drop out', async () => {
    ...
    expect((await store.getGroup(group.id))?.cancelled).toBe(false);
  ...
  it('reseats the people still coming instead of stranding them', async () => {
    ...
    await store.setRsvp(group.id, stayA!.id, 'accepted');
    ...
    for (const person of [stayA!, stayB!]) {
      const seat = await store.groupForEmployee(person.id, date, OFFICE);
  ...
  it('keeps a reply when someone is moved to another table', async () => {
    ...
    const seat = (await store.groupForEmployee(stayer.id, date, OFFICE))!;
    expect(seat.rsvps[stayer.id]).toBe('accepted');
  });

  it('marks a table that gained someone as needing a fresh invite', async () => {
    await planLunches();
    for (const group of await store.listGroups(date, OFFICE)) await store.markInviteSent(group.id);
    ...
    const hosts = (await store.listGroups(date, OFFICE)).filter(
      (g) => !g.cancelled && g.invitesSentAt === null,
    );
  ...
  it('never seats someone at a table with a teammate while reseating', async () => {
    ...
    for (const host of (await store.listGroups(date, OFFICE)).filter((g) => !g.cancelled)) {
  ...
  it('un-declining a dissolved table gets you a seat, not the old table back', async () => {
    ...
    await store.setRsvp(group.id, group.members[0]!.id, 'declined');
    await store.setRsvp(group.id, mindChanger.id, 'declined');
    expect((await store.getGroup(group.id))?.cancelled).toBe(true);

    await store.setRsvp(group.id, mindChanger.id, 'accepted');
    expect((await store.getGroup(group.id))?.cancelled).toBe(true);

    const seat = await store.groupForEmployee(mindChanger.id, date, OFFICE);
    expect(seat?.cancelled).toBe(false);
    expect(seat?.id).not.toBe(group.id);
  });
  ...
  it('keeps a table alive when one decline still leaves three people', async () => {
    ...
    await store.setRsvp(group.id, group.members[0]!.id, 'declined');
    expect((await store.getGroup(group.id))?.cancelled).toBe(false);
    expect((await store.getGroup(group.id))?.members).toHaveLength(4);
  });
  ...
  it('a cancelled table that was never announced needs no cancellation', async () => {
    ...
    expect((await store.getGroup(group.id))?.invitesSentAt).toBeNull();
    expect((await store.getGroup(group.id))?.cancellationSentAt).toBeNull();
  });

  it('records who could not be seated, and forgets it when cleared', async () => {
    const result = await planLunches();
    expect(await store.listUnmatched(date, OFFICE)).toEqual(result.unmatched);
  ...
  it('keeps today out of its own history so a re-run is not poisoned', async () => {
    ...
    expect((await store.listPastMatches()).some((m) => m.date === date)).toBe(true);
    const prior = (await store.listPastMatches()).filter((m) => m.date !== date);
