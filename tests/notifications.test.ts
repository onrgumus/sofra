...
describe('deliverPending', () => {
  it('sends an invite for every table that has not had one', async () => {
    const { store, date, transport, sent } = await setUp();

    const result = await deliverPending({ store, transport, from, date, officeId: OFFICE });
  ...
  it('is idempotent — running twice does not mail anyone twice', async () => {
    ...
    await deliverPending({ store, transport, from, date, officeId: OFFICE });
    const countAfterFirst = sent.length;
    const second = await deliverPending({ store, transport, from, date, officeId: OFFICE });
  ...
  it('does not send a cancellation twice', async () => {
    ...
    await deliverPending({ store, transport, from, date, officeId: OFFICE });
    sent.length = 0;
    const again = await deliverPending({ store, transport, from, date, officeId: OFFICE });
  ...
  it('says nothing to do for an office with no tables', async () => {
    const { store, transport, sent } = await setUp();
    const empty = upcomingWeekdays(5, todayInZone('Europe/Istanbul'))[4]!;

    const result = await deliverPending({ store, transport, from, date: empty, officeId: OFFICE });
